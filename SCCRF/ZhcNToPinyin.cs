...
namespace Game
{
    public class ConvertPY
    {
        ...
        public static int cntHighbit(byte b) {
            for (int i=0;i<8;i++) {
                if ((b >> i) == 0) return i;
            }
            return 8;
        }
        public static byte[] getGB2312(string str) {//假如默认是unicode编码
            List<byte> res = new List<byte>();
            int value;
            foreach (char m in str) {
                value = 0;
                if (m <= 0xff) { res.Add((byte)m);continue;}
                value = CodeTable.table[m.ToString()];
        ...
        public static string getPinyin(string str) {
            ...
            for (int i = 0; i < by.Length; i++)
            {
                ...
                {
                    if (i >= (by.Length - 1)) { result += (char)by[by.Length - 1]; break; }
                    int value = by[i] << 8 | by[i + 1];
                    for (int j = pyvalue.Length - 1; j >= 0; j--)
                    {
                        if ((pyvalue[j] & 0xffff) <= value)
                        {
                            result += pystr[j];
                            result += ",";
                            break;
